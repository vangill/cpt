...
class FrontendAsset extends AssetAbstract {
	...
	public function autoptimize_store_js( $url ) {
		...
		foreach ( $exclude_scripts as $script ) {
            if ( $exclude_script = strpos( $url, $script ) !== false ) break;
        }
		...
		if ( ! $this->is_login_page() ) {
			...
			if ( isset( $additional['style'] ) ) {
				...
				if ( ! empty( $style ) && $style != 'default' ) {
					wp_register_style( 'jnews-scheme', JNEWS_THEME_URL . '/' . Importer::$default_path . $style . '/scheme.css', null, $theme_version );
					if ( ! $this->is_debugging ) {
