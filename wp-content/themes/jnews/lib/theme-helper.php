		...
		foreach ( $socials as $social ) {
			...
			if ( ! empty( $icon ) ) {
				...
				if ( $class === 'jeg_line_chat' ) {
					...
					$socialstring[] = "<a href=\"{$social_url}\" target='_blank' rel='external noopener nofollow' class=\"{$class}\"><i class=\"{$icon}\"><span></span></i> {$title_string}</a>";
...
if ( ! function_exists( 'jnews_reset_license_server' ) ) {
	...
	function jnews_reset_license_server() {
		...
		$response = wp_remote_post( jnews_get_server_rest_url( 'resetLicense' ), $args );
...
if ( ! function_exists( 'jnews_get_server_rest_url' ) ) {
	/**
	 * Get server rest URL
	 *
	 * @param string $path
	 ...
	 * @return string
	 */
	function jnews_get_server_rest_url( $path ) {
		...
		if ( in_array( $path, $license_server, true ) ) {
...
if ( ! function_exists( 'jnews_get_twitch_data' ) ) {
    function jnews_get_twitch_data ( $name ) {
        ...
        if ( is_array( $twitch_token ) && ! empty( $twitch_token ) && time() < $twitch_token['expire'] ) {
            $search_response = wp_remote_get( "https://api.twitch.tv/helix/search/channels?query={$name}", [
                'headers'   => [
            ...
            if ( ! is_wp_error( $search_response ) ) {
                ...
                foreach ( $search_response->data as $data ) {
                    if ( $data->broadcaster_login === $name ) {
                        $search_response = wp_remote_get( "https://api.twitch.tv/helix/users/follows?to_id={$data->id}", [
                            'headers'   => [
                                'Authorization' => "Bearer {$twitch_token['token']}",
                                'client-id'     => get_theme_mod( 'jnews_twitch_client_id' ),
                            ],
